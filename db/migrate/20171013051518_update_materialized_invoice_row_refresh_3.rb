class UpdateMaterializedInvoiceRowRefresh3 < ActiveRecord::Migration[5.1]
  def up
    execute <<-SQL
  ...
  def down
    execute <<-SQL
      ...
      AS $BODY$
        BEGIN
          INSERT INTO materialized_invoice_rows (id, applicant_name, claim_number, case_number, case_number_without_prefix, work_order_number, order_type, entity_id, entity_name, display_name, account_id, territory_id, daisy_id, label_ids, ar_clerk_id)
            SELECT id, applicant_name, claim_number, case_number, case_number_without_prefix, work_order_number, order_type, entity_id, entity_name, display_name, account_id, territory_id, daisy_id, label_ids, ar_clerk_id
            FROM unmaterialized_invoice_rows
            WHERE unmaterialized_invoice_rows.id = ANY(invoice_ids)
          ON CONFLICT (id) DO UPDATE
            SET
