class UpdateMaterializedCaseRowRefresh1 < ActiveRecord::Migration[5.2]
  def up
    execute <<-SQL
      ...
      AS $BODY$
        BEGIN
          INSERT INTO materialized_case_rows (id, account_name, account_hivo_tag, billing_entity_name, billing_entity_id,
            requesting_attorney_name, user_to_contact_name, earliest_needed_by_date, earliest_created_at, latest_created_at
          )
            SELECT id, account_name, account_hivo_tag, billing_entity_name, billing_entity_id, requesting_attorney_name,
            user_to_contact_name, earliest_needed_by_date, earliest_created_at, latest_created_at
    ...
    execute <<-SQL
      CREATE OR REPLACE FUNCTION refresh_all_materialized_case_rows()
          RETURNS void
          LANGUAGE 'plpgsql'
          SECURITY DEFINER
      AS $BODY$
