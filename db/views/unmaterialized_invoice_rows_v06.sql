SELECT
  ...
  regexp_replace(COALESCE(cases.number, orders.case_number), '[[:alpha:]]', '', 'g') AS case_number_without_prefix,
  (
...
LEFT OUTER JOIN case_orders ON case_orders.order_id = orders.id AND case_orders."id" = ( SELECT min("id") FROM case_orders c1 WHERE c1.order_id = orders.id )
