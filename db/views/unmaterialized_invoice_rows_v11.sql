--- Gets the case and account regardless of work_order/request/voucher/add_set source
WITH invoice_parent_case AS (
  SELECT
    COALESCE(case_1.id, case_2.id) AS case_id,
    COALESCE(account_1.id, account_2.id) AS account_id,
  ...
  FROM
    ...
    LEFT OUTER JOIN accounts account_1 ON account_1.id = orders.account_id
    ...
    LEFT OUTER JOIN accounts account_2 ON account_2.id = vouchers.account_id
...
FROM
  invoices inv
  LEFT OUTER JOIN invoice_parent_case ON invoice_parent_case.invoice_id = inv.id
  LEFT OUTER JOIN accounts    ON accounts.id = invoice_parent_case.account_id
  LEFT OUTER JOIN cases       ON cases.id = invoice_parent_case.case_id
  LEFT OUTER JOIN work_orders ON work_orders.id = inv.parent_id AND inv.parent_type = 'WorkOrder'
  LEFT OUTER JOIN orders      ON orders.id = work_orders.order_id
