...
FROM
  invoices inv
  LEFT OUTER JOIN additional_sets ON additional_sets.id = inv.parent_id AND inv.parent_type = 'AdditionalSet'
  LEFT OUTER JOIN vouchers    ON vouchers.id = inv.parent_id AND inv.parent_type = 'Voucher'
  LEFT OUTER JOIN requests    ON (requests.id = inv.parent_id AND inv.parent_type = 'Request') OR (requests.id = additional_sets.request_id)
